...
class SignUpForm(UserCreationForm):
    ...
    first_name = forms.CharField(
        label=_("First name"), help_text=_("Required. Enter your first name."), max_length=30,
    )
    company_name = forms.CharField(
        label=_("Company name"), help_text=_("Required. Enter company name."), max_length=100,
    )
    phone_number = forms.CharField(
        label=_("Phone number"), help_text=_("Required. Enter your phone number."), max_length=30,
    )
...
class ResendActivationCodeForm(UserCacheMixin, forms.Form):
    ...
    def clean_email_or_username(self):
        ...
        if not settings.ENABLE_SEND_ACTIVATION_CODE_ANYTIME:
...
class InviteUserForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if user:
            raise ValidationError(_("You entered an existing email address."))

        return email


class InviteSignUpForm(SignUpForm):
    def __init__(self, company_name, email, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["company_name"].initial = company_name
