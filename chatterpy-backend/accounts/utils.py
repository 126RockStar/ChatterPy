...
def send_user_invitation_email(request, email):
    context = {
        "subject": _("User invitation"),
        "manager": request.user.email,
        "uri": request.build_absolute_uri(
            reverse(
                "accounts:invitation",
                kwargs={"account": request.user.account.id, "company": request.user.company_name, "email": email,},
            )
        ),
    }

    send_mail(email, "user_invitation", context)
