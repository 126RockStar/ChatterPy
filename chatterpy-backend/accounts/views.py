...
from .models import Account, Activation, ROLE_USER


class ManagerRequiredMixin(AccessMixin):
    """Verify that the current user is account manager."""
...
class ActivateView(View):
    """This view activates a user profile"""
...
class ResendActivationCodeView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, _("A new activation code has been sent to your email address."),
        )
...
class RestorePasswordView(GuestOnlyView, FormView):
    """This view is used to reset a user password via email."""
...
class ChangeEmailView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        # Send verification link to activate new email address
        if settings.ENABLE_ACTIVATION_AFTER_EMAIL_CHANGE:
            ...
            messages.success(
                self.request, _("To complete the change of email address, click on the link sent to it."),
            )
        else:
            user.email = email
            user.save()
...
class RestorePasswordConfirmView(BasePasswordResetConfirmView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, _("Your password has been set. You may go ahead and log in now."),
        )
...
class LogOutView(LoginRequiredMixin, BaseLogoutView):
    template_name = "pages/Bulk/sign-in.html"


class AddUserView(LoginRequiredMixin, ManagerRequiredMixin, FormView):
    template_name = "accounts/add_user.html"
    form_class = AddUserForm

    def get_form_kwargs(self):
        kwargs = super().get_form_kwargs()
        kwargs["company_name"] = self.request.user.company_name
        return kwargs
    ...
    def form_valid(self, form):
        ...
        if settings.ENABLE_USER_ACTIVATION_BY_MANAGER:
            user.is_active = False
        ...
        if settings.ENABLE_USER_ACTIVATION_BY_MANAGER:
            ...
            messages.success(
                request, _("You added new user. To activate the new user, follow the link sent to the mail."),
            )
        else:
            messages.success(request, _("You added new user successfully!"))

        return redirect("index")


class InviteUserView(LoginRequiredMixin, ManagerRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        send_user_invitation_email(request, form.cleaned_data["email"])

        messages.success(request, _("You sent the invitation email successfully."))
...
class InviteSignUpView(SignUpView):
    ...
    def get_form_kwargs(self):
        kwargs = super().get_form_kwargs()
        kwargs["company_name"] = self.kwargs["company"]
        kwargs["email"] = self.kwargs["email"]
        return kwargs
